...
# ===================== SHARED FOLDERS =====================
UPLOAD_ROOT = os.path.join(BASE_DIR, "uploaded")
...
def make_video(
    ...
    def task():
        try:
            ...
            # 🔒 SAFETY: ensure image is from shared images folder
            image_path_abs = os.path.abspath(image_path)

            if not image_path_abs.startswith(IMAGES_DIR):
                raise ValueError("Image must be inside project images folder")

            if not os.path.exists(image_path_abs):
                raise FileNotFoundError(f"Image not found: {image_path_abs}")
